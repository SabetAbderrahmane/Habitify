...
def recently_shown(
    ...
    return (datetime.now(timezone.utc) - shown_at) < timedelta(hours=cooldown_hours)


def mark_shown(conn, user_id: int, nudge_type: str, habit_name: str = "") -> None:
    conn.execute(
        """
    ...
    try:
        def add_nudge_if_allowed(
            *,
            ...
            if respect_history and recently_shown(
                conn,
                user_id,
                nudge_type,
                habit_name,
        ...
        for r in core:
            ...
            if (y_hit is None) and (db_hit is None):
                add_nudge_if_allowed(
                    ...
                    message=(
                        "You missed this core habit for 2 days. No pressure - "
                        "one small comeback today matters."
                    ),
                    priority=1,
        ...
        low_values = {"Low", "Bad", "Sad"}

        if low_moods.get(y_s) in low_values and low_moods.get(db_s) in low_values:
            add_nudge_if_allowed(
                nudge_type="low_mood_2days",
                title="Two low-mood days detected",
                message=(
                    "Try a tiny stabilizer today: water + 10-minute walk + early sleep."
                ),
                priority=2,
        ...
        if relapse_y and relapse_y["c"] > 0:
            add_nudge_if_allowed(
                ...
                title="Reset mode",
                message=(
                    "Yesterday had a slip. Today can be a reset - hit "
                    "'Urge Action Mode' if you feel the loop starting."
                ),
                priority=1,
            ...
            for ph in pack_habits:
                ...
                if started is None:
                    add_nudge_if_allowed(
                        ...
                        message=(
                            f"Quick win: add and log '{hname}' today "
                            "to kickstart your goal."
                        ),
                        priority=3,
