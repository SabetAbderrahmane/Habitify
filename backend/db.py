...
def migrate_habits(conn: sqlite3.Connection) -> None:
    ...
    # Check if we need to migrate: does habit_logs have a 'name' column?
    cur.execute("PRAGMA table_info(habit_logs)")
    table_info = cur.fetchall()
    has_name = any(col['name'] == 'name' for col in table_info)
    ...
    # 1. Get all unique habits from old habit_logs
    cur.execute("SELECT DISTINCT user_id, name FROM habit_logs")
    old_habits = cur.fetchall()
    
    for oh in old_habits:
        u_id = oh['user_id']
        name = oh['name']
        h_id = str(uuid.uuid4())
        
        # Create habit definition if not exists
        cur.execute(
            "INSERT OR IGNORE INTO habits (id, user_id, name) VALUES (?, ?, ?)",
            (h_id, u_id, name)
        )

    # 2. Re-create habit_logs with habit_id instead of name
    cur.execute("""
    ...
    # Copy data with habit_id lookup
    cur.execute("""
        INSERT INTO habit_logs_new (id, habit_id, user_id, progress, date, created_at)
        SELECT hl.id, h.id, hl.user_id, hl.progress, hl.date, hl.created_at
        FROM habit_logs hl
        JOIN habits h ON hl.user_id = h.user_id AND hl.name = h.name
    """)
    
    cur.execute("DROP TABLE habit_logs")
    cur.execute("ALTER TABLE habit_logs_new RENAME TO habit_logs")
